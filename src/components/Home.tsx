...
export default function Home({ setUsername, startMusic }: HomeProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.error('Firestore News Fetch Error:', error);
    });
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-full py-6 sm:py-12 px-4 relative">
      ...
      <div className="w-full max-w-[100%] overflow-hidden bg-tactical-amber py-2 sm:py-3 border-y border-black/10 my-6 sm:my-8 relative z-20 shadow-lg">
        <motion.div 
          animate={{ x: ["0%", "-33.33%"] }}
          transition={{ 
            duration: 20, 
        ...
        >
          {(newsItems.length > 0 ? [...newsItems, ...newsItems, ...newsItems] : [...Array(6)]).map((item, i) => (
            <div key={i} className="flex items-center gap-6 sm:gap-10 px-4">
              <Megaphone size={16} className="text-white animate-pulse shrink-0" />
              <span className="text-white font-mono text-[9px] sm:text-[12px] uppercase tracking-[0.1em] sm:tracking-[0.3em] font-bold">
                {item || "INITIALIZING_FEED..."}
              </span>
