...
import { Mail, Github, Instagram, Twitter, Facebook, ChevronDown, Menu, X, Moon, Sun, Trophy } from 'lucide-react';
...
export default function Navbar() {
  ...
  const contactLinks = [
    { icon: <Mail size={16} />, label: 'Email', href: 'mailto:contact@kemena.music' },
  ...
  const musicLinks = [
    { label: 'Spotify', href: 'https://open.spotify.com/artist/0SGTAjot9GShYwCQ69DfG2?si=UXzmZN8URKqmOi1SZzn4DA' },
    { label: 'YouTube', href: '#' },
    { label: 'Boomplay', href: '#' },
  ...
  return (
    <>
      <nav className="nav-fixed-hardened h-[80px] sm:h-[100px] bg-army-dark border-b border-black/5 shadow-md transition-colors duration-300 pointer-events-auto">
        <div className="h-full max-w-[1440px] mx-auto flex justify-between items-center px-4 sm:px-12 border-x border-transparent sm:border-black/5">
          {/* Left side: Logo */}
          ...
          <div className="hidden md:flex flex-1 justify-center items-center gap-[40px]">
            <div 
            ...
            >
              <button className="nav-link cursor-pointer">
                <span className="text-[10px] text-tactical-amber mr-1">01</span> MUSIC <ChevronDown size={14} className="group-hover:text-tactical-amber transition-colors" />
              </button>
              <AnimatePresence>
                {isMusicOpen && (
                  <motion.div 
            ...
            >
              <button className="nav-link cursor-pointer">
                <span className="text-[10px] text-tactical-amber mr-1">02</span> CONTACT <ChevronDown size={14} className="group-hover:text-tactical-amber transition-colors" />
              </button>
              <AnimatePresence>
                {isContactOpen && (
                  <motion.div 
